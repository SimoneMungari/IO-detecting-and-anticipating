...
from torch_geometric.nn import GCNConv, Node2Vec, SAGEConv, Linear, BatchNorm, to_hetero
...
def generate_node_features(G, nodes, latent_dim=32, feature_type="structural", edge_index=None):
    """
...
def align_graphs(G_inter, G_sim_train, G_sim_val):
    """
    Ensure same node ordering across graphs.
    """
    nodes = sorted(set(G_inter.nodes()) | set(G_sim_train.nodes()) | set(G_sim_val.nodes()))
...
with open('data/processed/{}/datasets_full_{}.pkl'.format(dataset, threshold_to_cut), 'rb') as f:
    datasets = pickle.load(f)

if not os.path.exists("checkpoints/gnn/{}_full".format(dataset)):
    os.makedirs("checkpoints/gnn/{}_full".format(dataset))


if not os.path.exists("data/processed/{}/features/".format(dataset)):
    os.makedirs("data/processed/{}/features/".format(dataset))
...
for i in datasets['splits']:
    ...
    G_sim_train = datasets['splits'][i]['final_graph']['train'].copy()
    G_sim_val = datasets['splits'][i]['final_graph']['val'].copy()

    G_sim_train = G_sim_train.subgraph(G_inter.nodes())
    G_sim_val = G_sim_val.subgraph(G_inter.nodes())

    if ablation == "all":
        nodes, node2idx, idx2node, inter_edges, sim_edges_train, sim_edges_val = align_graphs(G_inter, G_sim_train,
                                                                                              G_sim_val)
    elif ablation == "interactions":
        nodes, node2idx, idx2node, inter_edges, sim_edges_train, sim_edges_val = align_graphs(G_inter, G_inter,
                                                                                              G_inter)
    elif ablation == "similarities":
        nodes, node2idx, idx2node, inter_edges, sim_edges_train, sim_edges_val = align_graphs(G_inter, G_sim_train,
                                                                                              G_sim_val)

    print(G_inter.number_of_nodes(), G_inter.number_of_edges())
    print(G_sim_train.number_of_nodes(), G_sim_train.number_of_edges())
    print(G_sim_val.number_of_nodes(), G_sim_val.number_of_edges())

    set_seed(seeds[i])

    x_features_path = "data/processed/{}/features/G_inter_x_{}_{}_{}.pt".format(dataset, feature_type, i, latent_dim)
    x_features_path = "data/processed/{}/features/G_inter_x_full_{}_{}_{}.pt".format(dataset, feature_type, i, latent_dim)

    x_sim_train_features_path = "data/processed/{}/features/G_sim_train_x_{}_{}_{}_{}.pt".format(dataset, feature_type,
                                                                                           threshold_to_cut, i, latent_dim)
    x_sim_train_features_path = "data/processed/{}/features/G_sim_train_x_full_{}_{}_{}_{}.pt".format(dataset,
                                                                                                    feature_type,
                                                                                                    threshold_to_cut, i, latent_dim)

    x_sim_val_features_path = "data/processed/{}/features/G_sim_val_x_{}_{}_{}_{}.pt".format(dataset, feature_type,
                                                                                       threshold_to_cut, i, latent_dim)
    x_sim_val_features_path = "data/processed/{}/features/G_sim_val_x_full_{}_{}_{}_{}.pt".format(dataset, feature_type,
                                                                                                threshold_to_cut, i, latent_dim)


    train_edges = [(idx2node[x[0]], idx2node[x[1]]) for x in np.array(inter_edges)[datasets['splits'][i]['train']]]
    x = generate_node_features(G_inter.edge_subgraph(train_edges), nodes, feature_type="random", latent_dim=latent_dim)
    ...
    best_model_path = "checkpoints/gnn/{}_full/best_model_{}_{}_{}_{}_{}_{}.pt".format(dataset, i, feature_type, ablation, threshold_to_cut, hidden_dim, out_dim)

    # PyG data objects
    transform = T.Compose([T.ToUndirected(), T.AddRemainingSelfLoops(), T.RemoveDuplicatedEdges()])
    data_inter_train = nx_to_pyg_data(np.array(inter_edges)[datasets['splits'][i]['train']], len(nodes), x).to(device)
    ...
    pos_edge_index = []
    for edge in datasets['splits'][i]['train_positive_edges']:
        pos_edge_index.append((node2idx[edge[0]], node2idx[edge[1]]))

    for node in G_inter.nodes():
        if (node2idx[node], node2idx[node]) not in pos_edge_index:
            pos_edge_index.append((node2idx[node], node2idx[node]))
    ...
    val_roc_auc_best = train(model, data_inter_train, data_sim_train, pos_edge_index, pos_edge_index_val, neg_edge_index_val, best_model_path)
    ...
    with torch.no_grad():
        ...
        x = data_inter_train.x
        x_sim = data_sim_train.x
        edge_index_inter = data_inter_train.edge_index
        edge_index_sim = data_sim_train.edge_index
