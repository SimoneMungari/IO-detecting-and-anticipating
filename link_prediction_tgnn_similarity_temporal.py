...
for i in datasets['splits']:
    ...
    if dataset != "russia":
        ...
        train = datasets['edges_train_gnn'][['userid', 'interaction_with_userid']].to_numpy()
        val = datasets['edges_val_gnn'][['userid', 'interaction_with_userid']].to_numpy()
        G_inter_val = nx.from_edgelist(np.concatenate((train, val)))
    ...
    memory = TGNMemory(
        len(nodes),
        data_sim_train.msg.size(-1),
        memory_dim,
        time_dim,
        message_module=IdentityMessage(data_sim_train.msg.size(-1), memory_dim, time_dim),
    ...
    gnn = GraphAttentionEmbedding(
        ...
        msg_dim=data_sim_train.msg.size(-1),
    ...
    neighbor_loader = LastNeighborLoader(len(nodes), size=10, device=device)

    assoc = torch.empty(len(nodes), dtype=torch.long, device=device)

    val_roc_auc_best = train(memory, gnn, link_pred, None, None, neighbor_loader, train_loader, data_sim_train, pos_edge_index_val,
    ...
    with torch.no_grad():
        ...
        memory.update_state(data_sim_val.src, data_sim_val.dst, data_sim_val.t, data_sim_val.msg)
        neighbor_loader.insert(data_sim_val.src, data_sim_val.dst)

        z, last_update = memory(torch.arange(len(nodes)).to(device))

        z = gnn(z, last_update, data_sim_val.edge_index, data_sim_val.t.to(device),
                data_sim_val.msg.to(device))
