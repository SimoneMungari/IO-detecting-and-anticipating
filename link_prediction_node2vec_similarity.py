...
if not os.path.exists("checkpoints/node2vec/{}_{}_{}_full".format(dataset, similarity_network, threshold_to_cut)):
    os.makedirs("checkpoints/node2vec/{}_{}_{}_full".format(dataset, similarity_network, threshold_to_cut))
...
for i in datasets['splits']:
    ...
    G_sim = datasets['splits'][i][similarity_network]['train'].copy()
    ...
    if full:
        best_model_path = "checkpoints/node2vec/{}_{}_{}_full/best_model_{}.pt".format(dataset, similarity_network, threshold_to_cut, i)
    else:
        best_model_path = "checkpoints/node2vec/{}_{}_{}/best_model_{}.pt".format(dataset, similarity_network, threshold_to_cut, i)
    ...
    with torch.no_grad():
        ...
        print(f"Node2Vec run {i} AUC: {test_auc[-1]:.4f}, AP: {test_ap[-1]:.4f}", flush=True)
...
print(f"Node2Vec AUC: {auc_mean} +- {auc_std}, AP: {ap_mean} +- {ap_std}")
