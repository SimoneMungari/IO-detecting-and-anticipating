...
def reindex_graph(G_inter):
    nodes = sorted(set(G_inter.nodes()))
...
with open('data/processed/{}/datasets_full_0.3.pkl'.format(dataset), 'rb') as f:
    datasets = pickle.load(f)

if not os.path.exists("checkpoints/node2vec/{}_full".format(dataset)):
    os.makedirs("checkpoints/node2vec/{}_full".format(dataset))
...
nodes, node2idx, inter_edges = reindex_graph(G_inter)
...
for i in datasets['splits']:
    ...
    best_model_path = "checkpoints/node2vec/{}_full/best_model_{}.pt".format(dataset, i)

    pos_edge_index_train = []
    for edge in datasets['splits'][i]['train_positive_edges']:
        pos_edge_index_train.append((node2idx[edge[0]], node2idx[edge[1]]))
    ...
    for epoch in range(5000):
        ...
        with torch.no_grad():
            ...
            print(f"Node2Vec run {i}, epoch {epoch} Loss Train: {total_loss/len(loader):.4f}, VAL: AUC: {roc_auc:.4f}")
    ...
    with torch.no_grad():
        ...
        print(f"Node2Vec run {i} AUC: {test_auc[-1]:.4f}, AP: {test_ap[-1]:.4f}")
